...
> Screenshot placeholder: `.config/zellij/assets/zellij-dev-layout.png`
...
> Screenshot placeholder: `.config/zellij/assets/zellij-workspace-picker.png`
...
| `ft` | File pane | Yes |
| `keifu` | Git commit graph pane | Yes |
...
```sh
git clone <repository-url> ~/path/to/dotfiles
cd ~/path/to/dotfiles
make install
```

`make install` は、このリポジトリの `.config` を `~/.config` にリンクします。既存の `~/.config` が通常の directory の場合は `~/.config.backup` に移動されるため、実行前に内容を確認してください。
...
`layouts/dev.kdl` の次の command を、好みの TUI に置き換えられます。
